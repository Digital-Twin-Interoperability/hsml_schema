...
def register_entity(json_file_path, output_directory):
    ...
    # Store in MySQL
    cursor.execute(
        "REPLACE INTO did_keys (did, public_key, metadata, registered_by) VALUES (%s, %s, %s, %s)",
        (did_key, did_key, json.dumps(data), did_key)
    )
    db.commit()
    db.close()
    ...
    with open(private_key_output, "w") as private_key_file:
        private_key_file.write(f"Generated DID:key: {did_key}")
...
if __name__ == "__main__":
    ...
    result = register_entity(json_file_path, output_directory)
    print(result)
