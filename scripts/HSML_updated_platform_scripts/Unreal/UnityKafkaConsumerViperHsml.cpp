#include "UnityKafkaConsumerViper.h"
...
// Sets default values
AUnityKafkaConsumerViper::AUnityKafkaConsumerViper()
{
...
void AUnityKafkaConsumerViper::BeginPlay()
{
    Super::BeginPlay();
...
// Called every frame
void AUnityKafkaConsumerViper::Tick(float DeltaTime)
{
...
void AUnityKafkaConsumerViper::ReadAndUpdateActorFromJson()
{
    FString JsonString;
        ...
        if (JsonObject->TryGetArrayField(TEXT("position"), PositionArray))
        {
            for (const TSharedPtr<FJsonValue>& PropertyValue : *PositionArray)
            {
                ...
                if (PropertyObject.IsValid())
                {
                    ...
                    {
                        // Check the name and assign the appropriate value
